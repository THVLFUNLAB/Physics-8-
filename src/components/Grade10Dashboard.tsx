...
export default function Grade10Dashboard({ user, onStartPrescription, onStartExam, onDownloadPDF }: Grade10DashboardProps) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in zoom-in duration-500">
      {/* ── Hero Banner ── */}
      <HeroBanner />      {/* ── Bảng Xếp Hạng & Cá Nhân ── */}
      {user && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        <div className="space-y-3">
          <h2 className="text-sm font-black text-slate-500 uppercase flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-cyan-400 animate-ping pointer-events-none"></span>
        ...
        <div className="space-y-3">
          <h2 className="text-sm font-black text-slate-500 uppercase flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-violet-500 animate-pulse pointer-events-none"></span>
            Bản đồ YCCĐ Lớp 10
          </h2>
      ...
      <div className="bg-slate-900/50 backdrop-blur-md border border-slate-700/50 rounded-3xl p-8 relative overflow-hidden shadow-xl">
        ...
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-1 bg-gradient-to-r from-transparent via-cyan-500 to-transparent opacity-50 pointer-events-none" />
        <div className="absolute -top-20 -right-20 w-64 h-64 bg-cyan-500/5 rounded-full blur-3xl pointer-events-none" />
        <h3 className="text-3xl font-black flex items-center gap-3 mb-8 font-headline tracking-tight text-gradient-ocean relative z-10">
